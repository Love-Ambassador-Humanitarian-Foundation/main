...
const AdminProjectParticipants = ({ API_URL }) => {
    const { id: projectid } = useParams(); // Extracting the Project id
    ...
    const deleteParticipant = async (id) => {
        setIsLoading(true);
        try {
            const updatedParticipants = project.participants.filter(participantId => participantId !== id);
            await axios.put(`${API_URL}/api/projects/${projectid}`, {
                ...project,
                participants: updatedParticipants,
            });
            navigate(`/admin/projects/${projectid}/participants`)
            ...
            <div className='d-flex justify-content-between align-items-center p-2 m-2' style={{ backgroundColor: '#d7d7e9', borderRadius: '4px' }}>
                <Breadcrumb
                    items={[
                        ...
                        { href: `/#/admin/projects/${projectid}`, title: (<><ClockCircleOutlined /><span style={{ textDecoration: 'none' }}>{project?.title}</span></>) },
                        { title: (<><UserOutlined /><span>Participants</span></>) },
                    ]}
                />
                <Tooltip title='Add Participant'>
                    <Link to={`/admin/projects/${projectid}/participants/add`} state={{ project: project }} style={{ textDecoration: 'none' }}>
...
export default AdminProjectParticipants;
