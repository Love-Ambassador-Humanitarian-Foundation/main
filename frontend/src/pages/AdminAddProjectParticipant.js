...
const AddAdminProjectParticipant = ({ API_URL }) => {
    const { id } = useParams();
    const location = useLocation();
    const project = location.state?.project; // Default to empty participants if no project
    ...
    const saveEdit = async (currentformData) => {
        ...
        try {
            await axios.put(`${API_URL}/api/projects/${id}`, currentformData);
            message.success('Project details updated');
            
            navigate(`/admin/projects/${id}/participants`);
        } catch (error) {
            ...
            <div className='d-flex justify-content-between align-items-center p-2 m-2' style={{ backgroundColor: '#d7d7e9' }}>
                <Breadcrumb
                    items={[
                        ...
                        { href: '/#/admin/projects', title: (<><CalendarOutlined /><span>Projects</span></>) },
                        { href: `/#/admin/projects/${project?.id}`, title: (<><ClockCircleOutlined /><span style={{ textDecoration: 'none' }}>{project?.title}</span></>) },
                        { href: `/#/admin/projects/${project?.id}/participants`, title: (<><UserOutlined /><span>Participants</span></>) },
...
export default AddAdminProjectParticipant;
